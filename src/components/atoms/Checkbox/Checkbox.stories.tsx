  ...
  render: () => {
    ...
    return (
      <div className="space-y-3 p-4 border rounded-lg" style={{ width: '340px' }}>
        ...
        <Checkbox label="Email notifications" checked={vals.email}
          onCheckedChange={(v) => setVals(p => ({ ...p, email: v }))} id="perm-email" />
        <Checkbox label="SMS notifications" description="Requires verified phone number"
          checked={vals.sms} onCheckedChange={(v) => setVals(p => ({ ...p, sms: v }))} id="perm-sms" />
        <Checkbox label="Webhook events" description="POST to your configured endpoint"
          checked={vals.webhook} onCheckedChange={(v) => setVals(p => ({ ...p, webhook: v }))} id="perm-webhook" />
      </div>
    );
  },
};